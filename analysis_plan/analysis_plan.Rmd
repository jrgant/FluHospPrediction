...
emp <- readRDS(here::here("data", "empdat.Rds"))
setDT(emp)
```

# Motivation

- Influenza forecasting is still maturing science [@Reich2019-uk].
...
- Most of the newest work in influenza forecasting focuses on modeling influenza-like illness [@Reich2019-uk; @Biggerstaff2018-ns; @Biggerstaff2018-gt; @McGowan2019-ph; @Kandula2018-sq; @Brooks2015-fl].^[Finish reading McGowan 2019 and Kandula 2018.]

- While forecasting models for influenza-related hospitalization have been developed [@Kandula2019-tg], focus on hospitalizations has been more limited, and existing models have omitted variables that may result in improved forecasts.

- The number of empirical seasons available to model influenza-related hospitalizations is quite limited, with quality surveillance data existing back to the 2003--04 flu season.

## Objective/Rationale

- Our primary aim is to conduct a prediction-based simulation study to determine which of a set of candidate models and variable sets most closely fits a distribution of simulated hospitalization curves. These curves can be thought of as hypothetical flu seasons that could be, but perhaps have not yet, been realized [@Brooks2015-fl].

- By simulating an arbitrarily large population of curves, we can circumvent the low number of empirical hospitalization curves available to us, with the ultimate goal of identifying modeling approaches and variables that, if implemented, may improve in-season forecasts of influenza-related forecasts.
...
1. _Peak height_ = largest rate of hospitalizations in a single week
...
\hat{y} = \beta_0 + \beta_1 i
  + \beta_2 \left[ (i - I_1)^2_{+} - (i - I_k)^2_{+} \left]
...
tibble::tribble(
  ...
  "Temperature",
  "$T_{i}, T_{i-1}, T_{i-2}$",
  "Average historical temperature in the United States and Puerto Rico 2003--04 through 2018--19",
  "Temperatures in Fahrenheit, National Oceanic and Atmospheric Administration",

  "Absolute humidity",
  "$AH_{i}, AH_{i-1}, AH_{i-2}$",
  "",
...
1. For each candidate model, draw 1,000 bootstrap replicates of size $N_{train}$ from the training set.
   ...
   1. Within each bootstrap replicate, begin with a model containing all candidate variables, and select the best-fitting model using backwards stepwise selection based on the Akaike Information Criterion (AIC) [@Akaike1998-ll].^[Still need to read this chapter.]

   1. Calculate root mean squared error (RMSE) and relative bias (RB) in the training set.

1. Fit each of the candidate models to the training set, and calculate RMSE and RB.
...
$$RMSE = \left( \frac{1}{N} \sum_{i=1}^N (\hat{y}_j - y_{ij})^2 \right)^{0.5}$$

for each target parameter $j$ and hospitalization curve $i$.


$$RB = \frac{1}{N} \sum_{i=1}^N \frac{\text{RMSE}_j}{y_{ij}}$$
...
- We are not explicitly taking a forecasting approach, so the variables and models selected as part of this exercise may not translate to the forecasting case.
