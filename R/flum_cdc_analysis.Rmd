...
pkg_load <- c('Scotty', 'tidyverse', 'scales', 'purrr', 'broom', 'kableExtra', 'splines', 'glmnet', 'flumodelr', 'nnet')
#remotes::install_github('kmcconeghy/flumodelr')
...
wd.Datafiles <- "C:\\Users\\kevin\\Documents\\sanofi-fluhosp\\data"
...
df_mod <- df_cdc_dths_2 %>%
  select(y, year, month, week, yrweek_dt, epi, fluyr, ili, flupos, h1, h3, b, t_max, t_min,
         starts_with('sin'), starts_with('cos')) %>%
  mutate(y = round(y)) %>%
  mutate_at(vars(t_max, t_min, h1, h3),
...
for (i in unique(df_mod$fluyr)) {
  ...
  assign(paste0('fit_',i), glm(y ~ bs(week, 5) + bs(month, 2) + ili + 
                 sin_biann + sin_qtr + cos_qtr + sin_mth + cos_mth +
                 t_max + t_min + t_min_lg1 + t_min_lg2 + t_min_lg3 + 
                 h3 + h3_lg1 + h3_lg2 + h3_lg3 + b, family=poisson, data=df_iter))
  
  mod_fit <- get(paste0(paste0('fit_',i)))
  
  pred_test <- predict(mod_fit, newdata=mod_fit_dta[mod_fit_dta$fluyr==i, ], type='response')
  e <- pred_test - mod_fit_dta[mod_fit_dta$fluyr==i, ]$y
...
# Excess calculation  
```{r }
glm_fit <- glm(y ~ bs(week, 5) + bs(month, 2) + ili + 
                 sin_biann + sin_qtr + cos_qtr + sin_mth + cos_mth +
                 t_max + t_min + t_min_lg1 + t_min_lg2 + t_min_lg3 + 
...
for (i in unique(x_nnet_dta$fluyr)) {
  ...
  assign(paste0('fit_',i), nnet(y/max_y ~ bs(week, 5) + bs(month, 2) + epi + ili + flupos + 
                 sin_ann + cos_ann + sin_biann + cos_biann + sin_qtr + cos_qtr + sin_mth + cos_mth +
                 t_max + t_max_lg1 + t_max_lg2 + t_max_lg3 + 
                 t_min + t_min_lg1 + t_min_lg2 + t_min_lg3 + 
                 h1 + h1_lg1 + h1_lg2 + h1_lg3 +
                 h3 + h3_lg1 + h3_lg2 + h3_lg3 + b, 
               data=x_nnet_dta, size=2, maxit=1000, decay=1.0e-5, trace=F))
  
  mod_fit <- get(paste0(paste0('fit_',i)))
  
  pred_test <- predict(mod_fit, newdata=x_nnet_dta[x_nnet_dta$fluyr==i, ], type='raw')*max_y
  e <- pred_test - x_nnet_dta[x_nnet_dta$fluyr==i, ]$y
...
flunet <- nnet(y/max_y ~ bs(week, 2) + bs(month, 3) + 
                 ...
                 t_min + t_min_lg1 + t_min_lg2 + t_min_lg3 + 
                 h3 + h3_lg1 + h3_lg2 + h3_lg3 + b, 
               data=x_nnet_dta, size=3, maxit=1000, trace=F, decay=1.0e-5)  
