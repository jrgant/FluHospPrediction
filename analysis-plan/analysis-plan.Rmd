...
$$f^s (i) = b + \frac{\theta - b}{\text{max}_j f(j) - b} \left[ f \left( \frac{i - \mu}{v} + \genfrac{}{}{0pt}{}{\text{arg max }{j}} f (j) \right) - b \right]$$
...
The quadratic trend filter is fit using the following sampling scheme for each parameter represented in the model for hospitalization count ($y^s_i$) at each week. Note that all equations are either adapted from or appear in [@Brooks2015-fl].
