...
```{r flusurv-net_fig, echo = FALSE, fig.fullwidth = TRUE, fig.cap = "Empirical hospitalization curves, peak weeks, and peak number of cases --- 2003--2017 (Source: FluSurv-NET, CDC). Data excludes 2009--2010 pandemic influenza season."}
...
For each season $s$ and each week $i$, Brooks et al. conceptualize a seasonal influenza curve as some function plus noise:

$$y^s_i = f^s (i) + \epsilon^s_i, \epsilon \sim N(0, \tau^s),$$

where 

$$f^s (i) = b + \frac{\theta - b}{\text{max}_j f(j) - b} \left[ f \left( \frac{i - u}{v} + \genfrac{}{}{0pt}{}{\text{arg max }{j}} f (j) \right) - b \right]$$

and $b$ represents a seasonal "baseline" increase in cases which is used to designate officially the week of season onset. 
...
The quadratice trend filter is fit using the following sampling scheme for each parameter represented in the model for hospitalization count ($y^s_i$) at each week:
